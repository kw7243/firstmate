...
For numeric lock owners, `bin/fm-session-lock-lib.sh` decides session ownership against the whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, validates matching opaque Codex sandbox owners separately, and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
