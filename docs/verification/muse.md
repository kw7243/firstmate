...
The managed Muse launch clears foreign harness identity markers before the worker starts so marker precedence cannot override the versioned ancestry; `bin/fm-spawn.sh` owns the exact launch-time set.
