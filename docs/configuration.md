...
The Pi `/supervision-model` command settles both in one flow: it opens a selector over the full model catalog Pi already reports as available, plus a first "Follow main" entry, prepares only the selected provider when Pi exposes the required scoped constructor, and then opens a second picker for the branch's reasoning effort.
...
Firstmate requires an explicit provider-scoped `ModelRuntime` constructor; a Pi release without one rejects selected-model preparation before any all-provider runtime construction, so a catalog-model pick is not persisted and branch wakes return to main rather than loading or refreshing unrelated providers.
On a compatible release, runtime creation, the selected provider's offline refresh, and request-auth resolution are each bounded; a failed catalog-model selection preserves the current pin, while a branch build that cannot prepare the effective provider or a request-boundary freshness failure returns the wake to main.
A provider that exists only because an extension registered it inside the captain's session remains visible when Pi reports it, but selecting it fails safely before persistence because Pi exposes no public provider-scoped registration boundary.
On a release with that constructor, static Pi configuration, including model-specific headers, remains available through the isolated runtime without copying a live registration.
...
The picker keeps no effort catalog of its own; when the newly selected model or Follow-main model cannot be resolved, it offers no provider-specific levels and never consults the prior branch provider.
In that case, the command says that the branch's effective effort cannot be determined.
