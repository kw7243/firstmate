...
A network-disabled Codex sandbox can neither validate `gh auth status` nor read a macOS keyring-backed `gh auth token`; bootstrap suppresses `NEEDS_GH_AUTH` in that environment rather than asking for a login it cannot verify, while real GitHub call sites still fail closed if host auth is actually unavailable.
...
FM_LOCK_STALE_AFTER=2   # seconds before dead-pid or opaque Codex lock records can be reclaimed; mid-acquire locks keep at least 2s grace
