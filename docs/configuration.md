...
Pi 0.84.1 is the minimum release with the non-refreshing runtime boundary this isolation needs; older releases reject selected-model preparation before runtime construction, so a catalog-model pick is not persisted and branch wakes return to main rather than refreshing every provider.
Runtime creation, the selected provider's offline refresh, and request-auth resolution are each bounded; a failed catalog-model selection preserves the current pin, while a branch build that cannot prepare the effective provider or a request-boundary freshness failure returns the wake to main.
...
On compatible releases, static Pi configuration, including model-specific headers, remains available through the isolated runtime without copying a live registration.
