...
The retained exact-upstream implementation has one crash window for future comparison: when a prior-row stall marker exists, a crash after current-row progress is written but before the old marker is removed leaves every restart treating the still-stalled current row as already alerted until that row position changes.
...
Cross-home reads validate the seeded identity, operational-directory boundaries, and each local worker endpoint's task binding before trusting a recovery probe, and classify unavailable, malformed, or inconsistent structured state without reviving a parent event as current work; `bin/fm-fleet-snapshot.sh`'s header owns collection, cache selection, and unreadable-home behavior.
When only an owned child's current classification is unavailable, the home classification stays unknown while independently trustworthy structured decisions, holds, queued and landed records, endpoint identities, counts, provenance, inventory, and observability remain available; every other invalid path stays strict.
A keyed historical parent `working` or `paused` event for an affected child remains inconclusive rather than becoming current truth or an inventory contradiction.
Recovery evidence can preserve completed worker metadata as historical rather than unowned current work without weakening the unmatched-metadata rule.
The script header owns exact Done-role and recovery-verdict eligibility plus the additive evidence fields.
Bearings excludes endpoints marked as retained completion from current endpoint-health aggregation.
