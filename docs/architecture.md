...
When only an owned child's current classification is unavailable, the home classification stays unknown while a separate observability verdict preserves independently trustworthy structured in-flight rows, paths, decisions, holds, queued and landed records, recovery-grade endpoint identities, counts, and provenance; every inventory-invalid path stays strict and exposes none of those child-derived surfaces.
Metadata retained for a structured Done task is completed evidence rather than unowned current work only when the backend's recovery-grade state is dead or missing; alive, ambiguous, unreadable, or unverified unmatched metadata remains an inventory contradiction.
