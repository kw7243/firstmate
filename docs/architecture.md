...
Cross-home reads validate the seeded identity, operational-directory boundaries, and each worker endpoint's task binding before trusting a local recovery probe, use per-home time and output bounds, and classify unavailable, malformed, or inconsistent structured state as unknown rather than reviving a parent event as current work.
When only an owned child's current classification is unavailable, the home classification stays unknown while a separate observability verdict preserves independently trustworthy structured evidence; the script header owns the exact retained surfaces.
Every inventory-invalid path stays strict and exposes none of those child-derived surfaces.
...
Recovery evidence can preserve completed worker metadata as historical rather than unowned current work without weakening the fail-closed unmatched-metadata rule.
