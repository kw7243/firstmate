...
Providers and model-specific headers composed from Pi's static configuration remain available through the isolated selected-provider runtime, while the branch keeps its `noExtensions`, `noSkills`, and `noContextFiles` isolation.
