...
Supported Pi releases expose no public provider-scoped boundary for copying a provider registered only in main's runtime, so those providers are omitted from the picker and every selected or followed instance falls safely back to main before branch construction.
