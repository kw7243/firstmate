...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  Earlier conversations stay on disk under `state/branch-session/`, exactly as Pi keeps its own session files, and are never reopened as live branch context; the effort picker never inspects their provider when the selected or Follow-main model is unresolved.
  ...
  Stable non-drift provider failures during branch construction, request-auth freshness checks, or prompt settlement share one consecutive-error counter; the second failure latches the branch broken and surfaces a one-line health note only on that initial trip.
  Proven provider drift never advances that counter and stays retryable.
  ...
  A session replacement or branch model or effort change resets either failure state immediately, while a non-provider branch-construction failure remains broken until one of those resets and never enters cooldown probing.
...
The [configuration schema](configuration.md#pi-supervision-branch-model-and-effort-configsupervision-branch-model-configsupervision-branch-effort) owns the Pi capability boundary and its safe fallback before selected-provider runtime construction.
The branch keeps its `noExtensions`, `noSkills`, and `noContextFiles` isolation regardless of which supported selected provider that configuration owner admits.
The branch snapshots the effective selected model, routing, headers, request authorization, and native or custom stream composition and carries that snapshot through every asynchronous construction step; a bounded full request-auth comparison and a synchronous final composition check must both pass immediately before `state/.branch-session` can publish the candidate.
That full check repeats before cached reuse and at `before_provider_headers`; `before_provider_request` remains cache-key-only.
A model or effort selection change while a candidate is unpublished disposes it and rebuilds under the latest selection, while pre-existing cached drift rebuilds when the current provider can still be prepared.
Drift first observed during an asynchronous request-auth check or at the header boundary aborts before provider invocation, disposes the obsolete branch, and returns that wake to main without latching.
...
The same suite pins full-auth freshness and prior-pin preservation across the effort dialog, selection and request-auth races before branch-pointer publication, non-latching drift, unchanged-provider construction and freshness failures entering the shared cooldown, and exact holder-and-generation lease cleanup on provider fallback with non-provider controls.
