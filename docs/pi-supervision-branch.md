...
The [configuration schema](configuration.md#pi-supervision-branch-model-and-effort-configsupervision-branch-model-configsupervision-branch-effort) owns the Pi version boundary and its safe fallback before selected-provider runtime construction.
Supported Pi releases expose no public provider-scoped boundary for copying a provider registered only in main's runtime, so those providers remain visible in the full picker, but selecting one fails safely before persistence and following one falls safely back to main before branch construction.
On compatible releases, providers and model-specific headers composed from Pi's static configuration remain available through the isolated selected-provider runtime, while the branch keeps its `noExtensions`, `noSkills`, and `noContextFiles` isolation.
