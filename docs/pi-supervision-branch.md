...
A Pi release without an explicit provider-scoped `ModelRuntime` constructor keeps the full picker visible but refuses selected-value preparation before any all-provider construction, so main handles branch-eligible wakes.
Even on a compatible release, a provider registered only in main's runtime remains visible but unsupported unless Pi also exposes a provider-scoped registration-copy boundary.
On a release with both required boundaries, providers and model-specific headers composed from Pi's static configuration remain available through the isolated selected-provider runtime, while the branch keeps its `noExtensions`, `noSkills`, and `noContextFiles` isolation.
The branch snapshots the effective selected model, routing, headers, request authorization, and native or custom stream composition and rechecks that snapshot against main before reuse and again at `before_provider_headers`.
A change found before a turn invalidates the cached branch and rebuilds it when the current provider can still be prepared, while a change racing request authorization or the header boundary aborts before provider invocation, disposes the obsolete branch, and returns that wake to main; `before_provider_request` remains cache-key-only.
