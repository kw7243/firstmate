...
#     Metadata for a structured Done worker row is retained completion evidence
#     only when its recovery-grade endpoint is dead or missing; captain and
#     program rows never own retained child metadata. Endpoint rows carry the
#     retained_completed ownership flag.
...
prefetch_task_observations() {  # <meta> <id>
  ...
  local kind backend target endpoint_exists=null recovery_state=unverified agent_alive=not_checked generation_current=1
  ...
  printf 'endpoint_exists=%s\nrecovery_state=%s\nagent_alive=%s\n' \
    "$endpoint_exists" "$recovery_state" "$agent_alive" > "$endpoint_file" || current_rc=1
  return "$current_rc"
}
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    while IFS= read -r observation_line || [ -n "$observation_line" ]; do
      case "$observation_line" in
        ...
        recovery_state=*) recovery_state=${observation_line#*=} ;;
...
secondmate_home_summary_json() {  # <backlog-json-file> <tasks-json-file>
  jq -n \
    ...
    | ([ $completed[] | select(.hold_kind != "captain")
    ...
    | ([ $tasks[]
         ...
         | select(.id as $id | [$retained_completed[].id] | index($id) | not) ]) as $unknown_children
    | ([ $tasks[]
    ...
    | ($captain_holds_all
       + ([ $tasks[] as $t
            | select($t.id as $id | [$retained_completed[].id] | index($id) | not)
