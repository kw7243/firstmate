...
Pi's `/supervision-model` command opens a branch-only picker over the model catalog and thinking levels Pi reports; the [configuration schema](docs/configuration.md#pi-supervision-branch-model-and-effort-configsupervision-branch-model-configsupervision-branch-effort) owns compatibility, safe fallback, and no-pin behavior.
