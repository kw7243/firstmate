...
if (!(staleFailure instanceof Error) || !staleFailure.message.includes("provider registration changed during selected-model preparation")) {
  throw new Error(`the provider disappearance did not reject to main: ${String(staleFailure)}`);
}
if ((globalThis.__fmSessions ?? []).length !== 0) {
  throw new Error("the disappeared provider built a branch session");
}
if (existsSync(`${home}/config/supervision-branch-model`)) {
  throw new Error("the provider disappearance persisted a model pin");
}
if (sentToMain.length !== 0) {
  throw new Error(`the provider disappearance emitted branch success: ${JSON.stringify(sentToMain)}`);
}

registryModels.push(modelB);
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  const report = session.options.customTools.find((tool) => tool.name === "fm_branch_report");
  const recorded = await report.execute(
    "stable-provider-b",
    { task: "branch-driver", verdict: "routine", summary: "stable provider B rebuilt after disappearance" },
    undefined,
    undefined,
    {},
  );
...
const retry = dispatch("signal: retry provider after disappearance");
if (!retry.accepted) throw new Error("the provider disappearance latched the supervision branch");
await retry.settlement;
const sessions = globalThis.__fmSessions ?? [];
if (sessions.length !== 1 || sessions[0].model?.baseUrl !== modelB.baseUrl) {
  throw new Error(`the retry did not rebuild stable provider B: ${JSON.stringify(sessions.map((session) => session.model))}`);
}
if (!sentToMain.some((sent) => sent.message.content.includes("stable provider B rebuilt after disappearance"))) {
  throw new Error(`the stable retry produced no branch outcome: ${JSON.stringify(sentToMain)}`);
}
if (existsSync(`${home}/config/supervision-branch-model`)) {
  throw new Error("the stable retry created an unsolicited model pin");
